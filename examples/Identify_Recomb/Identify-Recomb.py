...
def identify(En):
    ...
    for i in range(0, max_timesteps, nskip):
        ...
        if (i % 5000 == 0): print("Time [ps] = %.2f"%(ts*dt-0.01))
    ...
    for mol in O2_mols:
        ...
        Oa = system.atoms_list[mol.idx[0] - 1]  # Assuming here that the dump coords is sorted
        Ob = system.atoms_list[mol.idx[1] - 1]  # Assuming here that the dump coords is sorted
...
if __name__ == '__main__':
    ...
    if write_reaction_count:
        ...
        for En in En_list:
            fw.write("%4.2f  %3d  %3d  %3d  %3d  %3d   %3d    %3d\n"%(En, len(reaction[En]['ER']), len(reaction[En]['LH']), len(reaction[En]['Phys']), len(reaction[En]['GP']), len(reaction[En]['Exch']), len(reaction[En]['None']), len(reaction[En]['Short']) ))
        fw.close()
