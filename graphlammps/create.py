...
class create:
    ...
    def __init__(self):
        ...
        self.num_rep = np.array(
            [10, 10, 2], dtype=int
        )  # Number of unit cells in each direction
        self.vac_height = np.array(
            [10.0, 120.0], dtype=float
        )  # Vaccuum height along the z direction [bottom, top]
        ...
        self.mark_all_layers = (
            False  # Do you want to mark each graphene layers individually?
        )
            ...
            for i in range(num_rep[0]):
                for j in range(num_rep[1]):
                    for k in range(num_rep[2]):
                        ...
                        system.atoms_list.append(
                            atom.atom(mass_C, idx + 1, atom_type, "C")
                        )
                        system.atoms_list[-1].pos = np.copy(pos)
                        system.atoms_list[-1].vel = np.copy(vel)
                        system.atoms_list[-1].layer = 2 * k
                        ...
                        if self.mark_corner:
                            if i == 0 and j == 0:
                                system.atoms_list[-2].corner = True
                            elif i == 0 and j == num_rep[1] - 1:
                                system.atoms_list[-1].corner = True
                            elif i == num_rep[0] - 1 and j == 0:
                                system.atoms_list[-2].corner = True
                            elif i == num_rep[0] - 1 and j == num_rep[1] - 1:
                                system.atoms_list[-1].corner = True
                        ...
                        if not self.is_graphene:
                            ...
                            atom_type = 2 * k + 2
                            system.atoms_list.append(
                                atom.atom(mass_C, idx + 2, atom_type, "C")
                            )
                            system.atoms_list[-1].pos = np.copy(pos)
                            system.atoms_list[-1].vel = np.copy(vel)
                            ...
                            system.atoms_list.append(
                                atom.atom(mass_C, idx + 3, atom_type, "C")
                            )
                            system.atoms_list[-1].pos = np.copy(pos)
                            system.atoms_list[-1].vel = np.copy(vel)
                            system.atoms_list[-1].layer = 2 * k + 1
            ...
            for i in range(num_rep[0]):
                for j in range(num_rep[1]):
                    for k in range(num_rep[2]):
                        ...
                        pos[0] = (
                            base_lattice[0]
                            + (3 / 4 + height_mod) * self.lat_a / 3**0.5
                        )
                        pos[1] = (
                            base_lattice[1] + (1 / 4 * 3**0.5) * self.lat_a / 3**0.5
                        )
                        ...
                        pos[0] = (
                            base_lattice[0]
                            + (5 / 4 + height_mod) * self.lat_a / 3**0.5
                        )
                        pos[1] = (
                            base_lattice[1] + (3 / 4 * 3**0.5) * self.lat_a / 3**0.5
                        )
                        pos[2] = base_lattice[2] + self.lat_c / 4
                        ...
                        pos[0] = (
                            base_lattice[0]
                            + (9 / 4 - 2 * height_mod) * self.lat_a / 3**0.5
                        )
                        pos[1] = (
                            base_lattice[1] + (3 / 4 * 3**0.5) * self.lat_a / 3**0.5
                        )
                        ...
                        pos[0] = (
                            base_lattice[0]
                            + (11 / 4 - 2 * height_mod) * self.lat_a / 3**0.5
                        )
                        pos[1] = (
                            base_lattice[1] + (1 / 4 * 3**0.5) * self.lat_a / 3**0.5
                        )
                        ...
                        if self.mark_corner:
                            if height_mod == 0:
                                if i == 0 and j == 0:
                                    system.atoms_list[-4].corner = True
                                elif i == 0 and j == num_rep[1] - 1:
                                    system.atoms_list[-3].corner = True
                                elif i == num_rep[0] - 1 and j == 0:
                                    system.atoms_list[-1].corner = True
                                elif i == num_rep[0] - 1 and j == num_rep[1] - 1:
                                    system.atoms_list[-2].corner = True
    ...
    def generate_velocities_lattice(self, system):
        ...
        if "reaxFF" in self.IP:
            self.unit_corr = 1e-5  # real  units
        else:
            self.unit_corr = 1e-2  # metal units
        ...
        for i in range(len(system.atoms_list)):
            ...
            if at.lattice:
                if (self.mark_corner and at.corner == False) or (
                    self.mark_corner == False
                ):
                    layer_count[at.layer] -= 1
    ...
    def write_struc(self, system, write_lmp=True, write_xyz=True):
        ...
        num_atoms = len(system.atoms_list)
        if system.num_atom_types <= 0:
            system.get_num_atom_types()
        ...
        if write_lmp:
            ...
            flmp.write(
                " Hexagonal lattice structure of dimension %d x %d x %d with a = %.4f and c = %.4f\n"
                % (
                    self.num_rep[0],
                    self.num_rep[1],
                    self.num_rep[2],
    ...
    def write_velocities(self, system):
        ...
        try:
            ...
            sys.exit(
                "No lmp file exists to write velocities. Write the atom positions first. "
            )
    ...
    def create_box(self, system):
        ...
        if self.ortho_box:
            ...
            system.lattice_pos[0] = np.array(
                [3 / 4 / 3**0.5 * self.lat_a, 0.25 * self.lat_a, 0.25 * self.lat_c]
            )
            system.lattice_pos[1] = np.array(
                [5 / 4 / 3**0.5 * self.lat_a, 0.75 * self.lat_a, 0.25 * self.lat_c]
            )
            system.lattice_pos[2] = np.array(
                [9 / 4 / 3**0.5 * self.lat_a, 0.75 * self.lat_a, 0.25 * self.lat_c]
            )
            system.lattice_pos[3] = np.array(
                [11 / 4 / 3**0.5 * self.lat_a, 0.25 * self.lat_a, 0.25 * self.lat_c]
            )
            ...
            system.lattice_pos = np.zeros((4, 3))
            system.lattice_pos[1] = np.array(
                [0.5 * self.lat_a, (0.5 / 3**0.5) * self.lat_a, 0]
            )
            system.lattice_pos[2] = np.array(
                [0.5 * self.lat_a, (0.5 / 3**0.5) * self.lat_a, 0.5 * self.lat_c]
            )
            system.lattice_pos[3] = np.array(
                [1.0 * self.lat_a, (1.0 / 3**0.5) * self.lat_a, 0.5 * self.lat_c]
            )
    ...
    def get_lat_const_from_file(self):
        ...
        if len(idx_Temp) > 0:
            ...
            lat_param[1] = np.interp(
                self.Lat_Temp, Lat_const[idx_pot][:, 0], Lat_const[idx_pot][:, 1]
            )
            lat_param[2] = np.interp(
                self.Lat_Temp, Lat_const[idx_pot][:, 0], Lat_const[idx_pot][:, 2]
            )
