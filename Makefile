...
black:
	...
	poetry run black --config pyproject.toml graphlammps examples

.PHONY: black-check
black-check:
	poetry run black --version
	poetry run black --diff --check --config pyproject.toml graphlammps examples

.PHONY: flake8
flake8:
	poetry run flake8 --version
	poetry run flake8 graphlammps examples

.PHONY: autoflake8-check
autoflake8-check:
	poetry run autoflake8 --version
	poetry run autoflake8 -r --exclude '__init__.py' graphlammps examples
	poetry run autoflake8 --check -r --exclude '__init__.py' graphlammps examples
